...
// rootHandler handles "/".
func (fe *Server) rootHandler(w http.ResponseWriter, r *http.Request) {
	token, err := r.Cookie(tokenCookieName)
	if err != nil || !verifyToken(token.Value, fe.config.publicKey) {
...
func (fe *Server) homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	tokenData, _ := decodeToken(token.Value, fe.config.publicKey)
	displayName := tokenData["name"].(string)
	username := tokenData["user"].(string)
	accountID := tokenData["acct"].(string)

	balance, err := fe.balanceReader.Get().GetBalance(r.Context(), accountID)
	if err != nil {
		logger.Error("Couldn't fetch balance", err)
	}
	txnHistory, err := fe.transactionHistory.Get().GetTransactions(r.Context(), accountID)
	if err != nil {
		logger.Error("Couldn't fetch transaction history", err)
	}
	contacts, err := fe.contacts.Get().GetContacts(r.Context(), username)
	if err != nil {
	...
	if err := templates.ExecuteTemplate(w, "index.html", map[string]interface{}{
		"ClusterName": fe.config.clusterName,
		"PodName":     fe.config.podName,
		"PodZone":     fe.config.podZone,
...
func (fe *Server) paymentHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	token, err := r.Cookie(tokenCookieName)
	if err != nil || !verifyToken(token.Value, fe.config.publicKey) {
	...
	if recipient == "add" {
		...
		if label != "" {
			...
			contact := contacts.Contact{
				...
				RoutingNum: fe.config.localRoutingNum,
				IsExternal: false,
			}
			err := fe.contacts.Get().AddContact(r.Context(), authenticatedAccountID, contact)
			if err != nil {
	...
	txn := model.Transaction{
		...
		FromRoutingNum: fe.config.localRoutingNum,
		ToAccountNum:   recipient,
		ToRoutingNum:   fe.config.localRoutingNum,
		Amount:         int64(paymentAmount),
		Timestamp:      time.Now(),
	}
	err = fe.ledgerWriter.Get().AddTransaction(r.Context(), r.FormValue("uuid"), authenticatedAccountID, txn)
...
func (fe *Server) depositHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	token, err := r.Cookie(tokenCookieName)
	if err != nil || !verifyToken(token.Value, fe.config.publicKey) {
	...
	if r.FormValue("account") == "add" {
		...
		if externalRoutingNum == fe.config.localRoutingNum {
...
func (fe *Server) loginHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		fe.loginGetHandler(w, r)
	} else if r.Method == http.MethodPost {
		fe.loginPostHandler(w, r)
	}
}
...
func (fe *Server) loginGetHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	...
	if registeredOauthClientID != "" && allowedOauthRedirectURI != "" && responseType == "code" {
		...
		if verifyToken(token.Value, fe.config.publicKey) {
	...
	} else if tokenErr == nil && verifyToken(token.Value, fe.config.publicKey) {
...
func (fe *Server) loginPostHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	err := fe.loginPostHelper(w, r)
	if err != nil {
...
func (fe *Server) loginPostHelper(w http.ResponseWriter, r *http.Request) error {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	loginReq := userservice.LoginRequest{
		Username: r.FormValue("username"),
		Password: r.FormValue("password"),
	}
	token, err := fe.userService.Get().Login(r.Context(), loginReq)
	if err != nil {
		return err
	}
	claims, err := decodeToken(token, fe.config.publicKey)
	if err != nil {
...
func (fe *Server) authCallbackHelper(state string, redirectURI string,
	token string, w http.ResponseWriter, r *http.Request, setConsentCookie bool) {
	client := &http.Client{Timeout: fe.config.backendTimeout}
	formData := url.Values{
		"state":    {state},
		"id_token": {token},
	}
...
func (fe *Server) consentHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		fe.consentGetHandler(w, r)
	} else if r.Method == http.MethodPost {
		fe.consentPostHandler(w, r)
	}
}

// consentGetHandler renders consent page.  Retrieves auth code if the user
// already logged in and consented.
func (fe *Server) consentGetHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	values := r.URL.Query()
	...
	if verifyToken(token.Value, fe.config.publicKey) {
		if consented == "true" {
			...
			fe.authCallbackHelper(state, redirectURI, token.Value, w, r, false)
			return
		}
...
func (fe *Server) consentPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	if consent == "true" {
		fe.authCallbackHelper(state, redirectURI, token.Value, w, r, true)
	} else {
...
func (fe *Server) signupHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		fe.signupGetHandler(w, r)
	} else if r.Method == http.MethodPost {
		fe.signupPostHandler(w, r)
	}
}

// signupGetHandler handles renders the signup page.
func (fe *Server) signupGetHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	token, err := r.Cookie(tokenCookieName)
	if err == nil && verifyToken(token.Value, fe.config.publicKey) {
		http.Redirect(w, r, "/home", http.StatusFound)
...
func (fe *Server) signupPostHandler(w http.ResponseWriter, r *http.Request) {
	logger := r.Context().Value(ctxKeyLogger{}).(*slog.Logger)
	...
	err := fe.userService.Get().CreateUser(r.Context(), creq)
	if err != nil {
	...
	if err := fe.loginPostHelper(w, r); err != nil {
		http.Redirect(w, r, "/login?msg=Login+Failed", http.StatusFound)
	}
}
