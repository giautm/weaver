...
func (s *Server) Init(ctx context.Context) error {
	...
	if env == "" || !slices.Contains(validEnvs, env) {
		s.Logger().Debug("ENV_PLATFORM is either empty or invalid")
		env = "local"
	}
	// Autodetect GCP.
	var err error
	...
	pubKeyBytes, err := ioutil.ReadFile(s.Config().PublicKeyPath)
	if err != nil {
...
func Serve(ctx context.Context, s *Server) error {
	...
	s.Logger().Debug("Frontend available", "addr", s.lis)
