...
func getEnv(name string) (string, error) {
	got := os.Getenv(name)
	if got == "" {
		return "", fmt.Errorf("%s environment variable not found", name)
	}
	return got, nil
}

func (i *impl) Init(context.Context) error {
	publicKeyBytes, err := ioutil.ReadFile(i.Config().PublicKeyPath)
...
func (i *impl) AddContact(ctx context.Context, authenticatedAccountID string, contact Contact) error {
	...
	i.Logger().Debug("Adding new contact to the database")
	if err = i.db.addContact(contact); err != nil {
		return fmt.Errorf("error adding contact: %s", err)
	}
	i.Logger().Info("Successfully added new contact.")
	return nil
}
